...
class Process(threading.Thread):
    ...
    def initTasks(self):
        ...
        #Angle aquisition
        #self.tasks.append([self.gui.taskToArduino, [self.gui.Arduino2, ["Motor","Nema17Vac","moveTo", str(config.getPosition(config.Nema17Vac, "Welding"))]]])
        self.tasks.append([self.gui.taskToArduino, [self.gui.Arduino2, ["Motor","Nema17Arr","moveTo", str(config.getPosition(config.Nema17Arr, "Spin"))]]])
        self.tasks.append([self.gui.taskToArduino, [self.gui.Arduino2, ["Motor","Nema17Vac","reset", "None"]]])
        self.tasks.append([self.gui.taskToArduino, [self.gui.Arduino2, ["Motor","Nema17Vac","moveTo", str(config.getPosition(config.Nema17Vac, "Welding"))]]])
        self.tasks.append([self.gui.taskToArduino, [self.gui.Arduino2, ["Motor","Nema17Arr","reset", "None"]]])
        self.tasks.append([self.gui.taskToArduino, [self.gui.Arduino1, ["Motor","Nema17Pos","moveTo", str(config.getPosition(config.Nema17Pos, "Alignment"))]]])
        self.tasks.append([self.getAngles, [config.batAngleBefore]])
        #Alignment of batteries
        for i in range(1,6):
            self.tasks.append([self.gui.taskToArduino, [self.gui.Arduino1, ["Motor","Nema17Ali","moveTo", str(config.getPosition(config.Nema17Ali, "Ali " + str(i)))]]])
            ...
            for j in range(1,6):
                self.tasks.append([self.gui.taskToArduino, [self.gui.Arduino1, ["Motor","48BJY28 " + str(j),"moveRad", str(config.batAngleBefore[i-1][j-1])]]])
            self.tasks.append([self.gui.taskToArduino, [self.gui.Arduino1, ["OnOff","Magnets","set", "On"]]])
            self.tasks.append([self.gui.taskToArduino, [self.gui.Arduino1, ["OnOff","PistonAli","set", "Off"]]])
            self.tasks.append([self.gui.taskToArduino, [self.gui.Arduino1, ["","","wait", "1000"]]])
            for j in range(1,6):
                self.tasks.append([self.gui.taskToArduino, [self.gui.Arduino1, ["Motor","48BJY28 " + str(j),"reset", "None"]]])
            self.tasks.append([self.gui.taskToArduino, [self.gui.Arduino1, ["OnOff","Magnets","set", "Off"]]])
        #Welding
        for i in range(1,6):
            self.tasks.append([self.gui.taskToArduino, [self.gui.Arduino1, ["Motor","Nema17Pos","moveTo", str(config.getPosition(config.Nema17Pos, "Welding "+ str(i)))]]])
            self.tasks.append([self.gui.taskToArduino, [self.gui.Arduino2, ["Motor","Nema17Arr","moveTo", str(config.getPosition(config.Nema17Arr, "Spin"))]]])
            self.tasks.append([self.gui.taskToArduino, [self.gui.Arduino2, ["Motor","Nema17Vac","moveTo", str(config.getPosition(config.Nema17Vac, "Arrester"))]]])
            self.tasks.append([self.gui.taskToArduino, [self.gui.Arduino2, ["Motor","Nema17Arr","moveTo", str(config.getPosition(config.Nema17Arr, "Welding"))]]])
