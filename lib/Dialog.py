...
class Dialog(tk.Toplevel):
    ...
    def __init__(self, gui,  **kwargs):
        ...
        if 'title' in self.args: self.title(self.args.get('title'))
        self.result = None
        body = tk.Frame(self)
...
class dialogNumber(Dialog):
    def body(self, master):
        ...
        if 'value' in self.args:
            ...
            if self.value[0]: self.textInput.insert(tk.END, self.value[0])
...
class motorConfig(Dialog):
    def body(self, master):
            ...
            if 'listOfPositions'in self.args: self.pos = self.args.get('listOfPositions')
            else: self.pos = list()
    ...
    def newPosition(self):
        ...
        if position[0] and position[1]: self.pos.append(position)
        self.mUpdate()
...
class editPos(Dialog):
    def body(self, master):
        if 'position'in self.args: self.position = self.args.get('position')
        tk.Label(master, text="Name:").grid(row=0)
        tk.Label(master, text="Steps:").grid(row=1)

        self.entryName = tk.Entry(master)
        if self.position[0]: self.entryName.insert(tk.END, self.position[0])
        self.entryValue = tk.Entry(master)
        if self.position[1]: self.entryValue.insert(tk.END, self.position[1])
